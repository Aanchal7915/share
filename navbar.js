...
export default function NavBar(){
  ...
  const NavLink = ({ to, children, icon: Icon }) => (
    <Link 
      to={to} 
      className="flex items-center gap-1 text-sm font-medium text-gray-600 hover:text-primary transition duration-150 p-2 rounded-lg hover:bg-gray-50"
    >
  ...
  return (
    // Add shadow-md, make it sticky, and add a max-width container for a cleaner look
    <header className="sticky top-0 z-50 bg-white shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between py-3">

          {/* Logo and title - Use a brighter color for the logo, bolder text */}
          <div className="flex items-center gap-3">
            {/* Using a deeper indigo and larger size for the logo icon */}
            <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white font-extrabold text-sm shadow-lg">SM</div>
          ...
          <nav className="flex items-center gap-6">
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={logout} 
                  className="flex items-center gap-1 px-4 py-2 text-sm rounded-lg bg-red-500 text-white font-semibold hover:bg-red-600 transition duration-150 shadow-md"
                >
